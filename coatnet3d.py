...
class Attention(nn.Module):
    def __init__(self, inp, oup, image_size, heads=8, dim_head=32, dropout=0.):
        ...
        #     torch.zeros((2 * self.ih - 1) * (2 * self.iw - 1) * (2 * self.id - 1), heads))
        #
        # coords = torch.meshgrid((torch.arange(self.ih), torch.arange(self.iw), torch.arange(self.id)))
    ...
    def forward(self, x):
        ...
        #     relative_bias, '(h w) c -> 1 c h w', h=self.ih*self.iw*self.id, w=self.ih*self.iw*self.id)
...
class CoAtNet(nn.Module):
    def __init__(self, image_size, in_channels, num_blocks, channels, num_classes=1000, block_types=['C', 'C', 'T', 'T']):
        ...
        self.s2 = self._make_layer(
            block[block_types[1]], channels[1], channels[2], num_blocks[2], (ih // 8, iw // 8, id // 8))
        self.s3 = self._make_layer(
            block[block_types[2]], channels[2], channels[3], num_blocks[3], (ih // 16, iw // 16, id // 16))
        self.s4 = self._make_layer(
            block[block_types[3]], channels[3], channels[4], num_blocks[4], (ih // 32, iw // 32, id // 32))

        self.pool = nn.AvgPool3d(id // 32, 1)
